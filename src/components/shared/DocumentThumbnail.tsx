...
export const DocumentThumbnail: React.FC<DocumentThumbnailProps> = ({ material, size = "lg" }) => {
  ...
  if (size === "lg") {
    return (
      <div className="aspect-[16/10] w-full bg-slate-100 dark:bg-zinc-900 border-b border-slate-100 dark:border-zinc-800/80 relative overflow-hidden group-hover:opacity-95 transition-opacity flex items-center justify-center">
        {material.filePath ? (
          <div className="w-full h-full pointer-events-none select-none overflow-hidden origin-top">
            <iframe
              src={`${material.filePath}#page=1&toolbar=0&navpanes=0&scrollbar=0&view=FitH`}
              className="w-full h-[180%] border-none pointer-events-none scale-100"
              style={{ overflow: "hidden" }}
              scrolling="no"
            />
          </div>
        ) : (
          <div className="w-full h-full p-4 flex flex-col justify-between relative bg-white dark:bg-zinc-800">
  ...
  return (
    <div className="w-10 h-14 rounded-lg bg-slate-100 dark:bg-zinc-900 border border-slate-200 dark:border-zinc-700 relative overflow-hidden shadow-sm shrink-0 flex items-center justify-center">
      {material.filePath ? (
        ...
        <div className="w-full h-full p-1 flex flex-col justify-between relative bg-white dark:bg-zinc-800">
          {/* Left page margin line */}
